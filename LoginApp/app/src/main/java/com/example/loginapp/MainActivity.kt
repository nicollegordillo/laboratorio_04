    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnLogin.setOnClickListener {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                if (jwt != null) {
                    if(jwt.equals("error_bad_credentials")){
                        myTextView.text="Usuario no registrado"
                        println("hi")
                    }
                    else if(jwt.equals("error_bad_password")){
                        myTextView.text="Contraseña incorrecta"
                    }
                    else{
    ...
    suspend fun loginAndStoreJwt(context: Context, username: String, password: String): String? {
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (response.isSuccessful && responseBody != null && !responseBody.equals("error_bad_credentials")) {
                ...
                if(responseBody.equals("error_bad_credentials")){
                    val jwt = responseBody
                    return@withContext jwt
                }
                else if(responseBody.equals("error_bad_password")){
                    val jwt = responseBody
                    return@withContext jwt
                }
                else{
